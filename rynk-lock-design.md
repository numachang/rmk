...
- `BootloaderJump` — firmware replacement.
...
The locked set is expressed as one explicit `matches!` list next to the dispatch match in `rmk/src/host/rynk/mod.rs` — plain control flow, no macro, mirroring the command-table comment.
...
- **Any session end** — `run_session` calls `locker.lock()` on every return path (wrap the loop body so there is one call in one place). This kills the hole Vial has today, where unlock persists after the tool disconnects and a later drive-by page with a stale port grant inherits it.
...
**Cross-session interaction (state is global):** unlocking via USB unlocks the BLE session too — accepted, it's the same physical authorization. Conversely, *any* session ending relocks everyone: a laptop BLE disconnect mid-USB-flash forces the USB tool to re-run the unlock ceremony. Rare, benign (hold the keys again), and fail-closed beats maintaining a session refcount.
...
Reuse and generalize the existing `[host]` section (`HostConfig`, `rmk-config/src/lib.rs` ~line 842) — `unlock_keys` is already protocol-neutral there:
...
- Rename `vial_insecure` → `insecure` with `#[serde(alias = "vial_insecure")]` so one flag serves both protocols and existing TOMLs keep parsing (`deny_unknown_fields` would otherwise reject them).
...
- Plumbing: add `LockConfig { unlock_keys: &'static [(u8, u8)], insecure: bool, write_requires_unlock: bool }` to `rmk/src/config` (under the `rynk` feature), a new field on `RmkConfig`, and a codegen block in `rmk-macro/src/codegen/keyboard_config.rs` mirroring `expand_vial_config`. `RynkService::new(keymap, config)` already receives `&RmkConfig` (currently `_config`) — no signature change. Via keeps reading its existing `VialConfig` fields; migrating it to `LockConfig` is the deferred `vial_lock` cleanup.
...
1. `payload/system.rs`: reshape `LockStatus` (add `key_positions`, rename `awaiting_keys` → `unlocking`), delete `UnlockChallenge`, `UNLOCK_KEYS_SIZE` 2 → 4; update round-trip + max-size-bound tests.
...
3. `rmk/src/config`: `LockConfig`; `RmkConfig` field; `rmk-config`: `insecure` rename + alias, `write_requires_unlock`, unlock-key count/bounds validation; `rmk-macro/src/codegen/keyboard_config.rs`: emit `LockConfig` when `rynk_enabled`.
4. `host/rynk/mod.rs`: `RynkService` gains `locker: HostLock` (built in `new` from config); `requires_unlock(cmd)` explicit `matches!` list + gate at the top of `dispatch`; relock on every `run_session` return path (single call site).
...
7. Tests, in-crate:
   - `host/lock.rs` unit tests: arm/refresh/expiry of the window, partial → full key hold, empty-keys never unlocks, `insecure` survives `lock()`.
